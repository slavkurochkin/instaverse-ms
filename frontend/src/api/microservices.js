...
export const getUserProfile = async (userId) =>
  api.get(`/api/auth/profile/${userId}`);
export const validateToken = async () => api.get('/api/auth/validate-token');

// Legacy compatibility - map old endpoints to new ones
export const fetchStoriesByTag = async (tag) =>
  api.get(`/api/stories?tag=${tag}`);
export const fetchAllTags = async () => api.get(`/api/stories`); // Can be enhanced in Story Service
...
export const getUserProfiles = async () => api.get(`/api/auth/profile`);
